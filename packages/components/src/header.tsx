'use client';

import { useAuthStore } from '@zchemacraft/stores/auth-store';
import { User } from '@zchemacraft/types';
import Link from 'next/link';
import { UserButton } from './core/user-button';
import { Badge } from './ui/badge';

export function Header() {
  const user = useAuthStore(state => state.user);
  return (
    <header className="py-4 px-4 sm:px-8 md:px-16 bg-black w-full">
      <nav className="mx-auto flex items-center justify-between">
        <span className="flex space-x-2">
          <h1 className="font-medium text-xl md:text-2xl">Zchemacraft.</h1>
          <Badge className="bg-purple-900/50 text-xs px-2 text-purple-300 flex items-center rounded-full">
            beta
          </Badge>
        </span>

        <div className='flex items-center space-x-5'>
          <Link href={user ? '/mock-api' : '/auth/login'}>
            <button className="bg-slate-800 no-underline group cursor-pointer relative shadow-2xl shadow-zinc-900 rounded-full p-px text-xs font-semibold leading-6  text-white inline-block">
              <span className="absolute inset-0 overflow-hidden rounded-full">
                <span className="absolute inset-0 rounded-full bg-[image:radial-gradient(75%_100%_at_50%_0%,rgba(147,51,234,0.6)_0%,rgba(147,51,234,0)_75%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
              </span>

              <div className="relative flex space-x-2 items-center z-10 rounded-full bg-zinc-950 py-1 text-base px-6 ring-1 ring-white/10 ">
                <span>Mock API</span>
              </div>
              <span className="absolute -bottom-0 left-[1.125rem] h-px w-[calc(100%-2.25rem)] bg-gradient-to-r from-purple-800/0 via-purple-800/90 to-purple-800/0 transition-opacity duration-500 group-hover:opacity-40" />
            </button>
          </Link>

          {user ? <UserButton user={user as User} /> : null}
        </div>
      </nav>
    </header>
  );
}
